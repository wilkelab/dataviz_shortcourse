...
lm_data <- iris |>
  nest(data = -Species) |>
  mutate(
    fit = map(data, ~lm(Sepal.Width ~ Sepal.Length, data = .x)),
    tidy_out = map(fit, tidy)
  ) |>
  unnest(cols = tidy_out) |>
  select(-data, -fit) |>
...
```{r}
# half eyes
ggplot(lm_data, aes(x = estimate, y = Species)) +
  stat_halfeye(
    aes(dist = dist_normal(
      mu = estimate, sigma = std.error
    )),
...
```{r}
# fit the correct gam() model
fit <- gam(mpg ~ s(disp, bs = "cs"), data = mtcars, method = "REML")

# new data table with x values only, for model prediction
mtcars_new <- tibble(
  disp = seq(from = 60, to = 480, length.out = 100)
) |>
  mutate(.row = row_number()) # needed to join in fitted samples

# fitted_values() returns mean and confidence band
fv <- fitted_values(fit, data = mtcars_new)

# fitted_samples() returns independent samples from the fitted model
fs <- fitted_samples(fit, data = mtcars_new, n = 30, seed = 10) |> 
  left_join(mtcars_new, by = ".row")

anim_obj <- ggplot(mtcars, aes(disp)) + 
  geom_ribbon(
    data = fv, aes(ymin = .lower_ci, ymax = .upper_ci),
    fill="grey70", color = NA, alpha = 1/2
  ) +
  geom_point(aes(y = mpg), color = "grey30", size = 1.5) +
  geom_line(
    data = fs,
    aes(y = .fitted, group = .draw),
    color = "#0072B2", linewidth = 0.3
...
```{r}
# let's first create a new dataframe that has both linear and log detrended results
fmhpi_trends <- fmhpi |>
  nest(data = -state) |>
  mutate(
    # fit in log space (never fit exponentials in linear space!)
  ...
  select(-fit_log, -fit_lin) |>
  unnest(cols = c(data, hpi_trend_log, hpi_trend_lin))

# log-space detrended results for four other states
fmhpi_trends |>
  filter(state %in% c("New York", "Colorado", "South Dakota", "Florida")) |>
  ggplot(aes(date_dec, hpi / hpi_trend_log)) +
  geom_hline(yintercept = 1, linewidth = 0.2) +
  geom_line(color = "#0072B2") +
  xlab("Year") +
  scale_y_log10(name = "HPI detrended") +
  facet_wrap(~state)

# and linear space detrended results for the same states
fmhpi_trends |>
  filter(state %in% c("New York", "Colorado", "South Dakota", "Florida")) |>
  ...
  geom_line(color = "#0072B2") +
  xlab("Year") +
  scale_y_continuous(name = "HPI detrended") +
  facet_wrap(~state)
```
...
# plot
temps_detrended |>
  rename("temperature" = response) |>
  pivot_longer(-date_dec, names_to = "component", values_to = "response") |>
  mutate(
    component = fct_relevel(component, "temperature", "trend", "seasonal", "remainder")
  ) |>
  ggplot(aes(date_dec, response)) +
  geom_line() +
  facet_wrap(~component, scales = "free_y", ncol = 1)
...
```{r}
# decomposition of precipitation
...
# perform STL analysis (Seasonal Decomposition of Time Series by Loess)
precip_stl <- stl(precip_ts, s.window = 7)
# extract components and put together into one table
precip_detrended <- tibble(
  date_dec = temps_reduced$date_dec,
  response,
  seasonal = t(precip_stl$time.series)[1, ],
  trend = t(precip_stl$time.series)[2, ],
  remainder = t(precip_stl$time.series)[3, ]
)

# plot
precip_detrended |>
  rename("precipitation" = response) |>
  pivot_longer(-date_dec, names_to = "component", values_to = "response") |>
  mutate(
    component = fct_relevel(component, "precipitation", "trend", "seasonal", "remainder")
  ) |>
  ggplot(aes(date_dec, response)) +
  geom_line() +
  facet_wrap(~component, scales = "free_y", ncol = 1)
...
ggplot(cars93) +
  ...
  geom_smooth(
    method = "gam",
    # Gaussian process spline, three knots
    formula = y ~ s(x, k = 3, bs = 'gp')
  )
ggplot(cars93) +
  aes(x = Price, y = Fuel.tank.capacity) +
  geom_point() + theme_bw(14) +
